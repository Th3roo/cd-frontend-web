...
const StatusPanel: React.FC<StatusPanelProps> = ({ player, gameState, target, globalTick = 0 }) => {
  return (
    <div className="bg-neutral-900 border-b border-neutral-700 p-4 flex justify-between items-center h-24">
      ...
      <div className="w-1/3">
        ...
        <div className="flex gap-2 w-full">
            <div className="flex-1"><ProgressBar value={player.stats.hp} max={player.stats.maxHp} color="bg-red-500" label="HP" /></div>
            <div className="flex-1"><ProgressBar value={player.stats.stamina} max={player.stats.maxStamina} color="bg-yellow-500" label="STM" /></div>
        </div>
      </div>

      {/* Game State Indicator */}
      <div className="flex flex-col items-center">
        <div className={`px-4 py-1 rounded border ${gameState === GameState.COMBAT ? 'border-red-500 text-red-500 animate-pulse' : 'border-green-500 text-green-500'}`}>
      ...
      <div className="w-1/3 flex flex-col items-end">
        {target && !target.isDead ? (
          <div className="w-full text-right">
            ...
            <ProgressBar value={target.stats.hp} max={target.stats.maxHp} color="bg-purple-600" label="Здоровье Врага" />
          </div>
        ) : (
