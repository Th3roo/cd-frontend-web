...
import { 
    GameWorld, Entity, EntityType, GameState, Position, 
    GameAction, GameEvent, LogType, ItemType 
} from '../types';
import { generateDungeon, generateTown } from '../services/dungeonGenerator';
import { SYMBOLS, COLORS, STAMINA_COST, STAMINA_REGEN, PRICES, TIME_COST, MAP_WIDTH, MAP_HEIGHT } from '../constants';
...
export class GameEngine {
    ...
    public init(level: number, existingPlayer?: Entity): GameEvent[] {
        ...
        events.push({
            type: 'LEVEL_CHANGE',
            text: level === 0 ? "Вы прибыли в Тихую Гавань." : `Вы спустились на уровень ${level}.`,
    ...
    public dispatch(action: GameAction): GameEvent[] {
        ...
        switch (action.type) {
            case 'MOVE':
                const moveResult = this.handleMove(action.payload.dx, action.payload.dy);
            ...
            case 'PICKUP':
                ...
                if (pickResult.some(e => e.logType === LogType.ERROR)) return pickResult;
            ...
            case 'USE':
                ...
                if (useResult.some(e => e.logType === LogType.ERROR)) return useResult;
    ...
    public async handleSocialMove(text: string): Promise<GameEvent[]> {
        ...
        const npc = this.getNearbyNPC() || this.getVisibleEntities().find(e => e.isHostile && !e.isDead);

        if (!npc) {
            return [{ type: 'LOG', text: "Вас никто не слышит.", logType: LogType.INFO }];
        }

        const events: GameEvent[] = [];
        events.push({ type: 'LOG', text: `Вы: "${text}"`, logType: LogType.SPEECH });

        // 2. Сформировать контекст
        const dist = Math.floor(Math.sqrt((npc.pos.x-p.pos.x)**2 + (npc.pos.y-p.pos.y)**2));
        ...
        if (!result) {
            events.push({ type: 'LOG', text: "NPC не реагирует (ошибка связи с сознанием).", logType: LogType.INFO });
            return events;
        }
        ...
        events.push({ type: 'LOG', text: `${npc.name}: "${reactionText}"`, logType: LogType.SPEECH });
        ...
        if (result.success && result.newState) {
            ...
            if (result.newState === 'IDLE' && npc.isHostile) {
                npc.isHostile = false;
                events.push({ type: 'GAME_STATE_CHANGE', text: `${npc.name} опускает оружие.`, logType: LogType.SUCCESS });
            }
            if (result.newState === 'FLEEING') {
                events.push({ type: 'LOG', text: `${npc.name} бросается наутек!`, logType: LogType.SUCCESS });
            }
        } else {
             // Если неудача, но NPC был не враждебен, он может обидеться (опционально)
             events.push({ type: 'LOG', text: "Это не подействовало.", logType: LogType.INFO });
        }
    ...
    private runGameLoop(): GameEvent[] {
        ...
        while (true) {
            ...
            if (this.gameState === GameState.GAME_OVER) break;
    ...
    private processSingleNPCTurn(npc: Entity): GameEvent[] {
        ...
        // Ensure state
        if (!npc.aiState) npc.aiState = npc.isHostile ? 'AGGRESSIVE' : 'IDLE';
        ...
        if (npc.aiState === 'FLEEING') {
            ...
            const dist = Math.sqrt(Math.pow(p.pos.x - npc.pos.x, 2) + Math.pow(p.pos.y - npc.pos.y, 2));
            const canSee = this.hasLineOfSight(npc.pos.x, npc.pos.y, p.pos.x, p.pos.y);
            ...
            if (dist > 15) {
                ...
                if (p.stats.hp <= 0) {
                    ...
                    events.push({ type: 'DEATH', text: "ВЫ ПОГИБЛИ.", logType: LogType.ERROR });
                }
    ...
    private moveTowards(npc: Entity, targetX: number, targetY: number): number {
        let dx = targetX - npc.pos.x;
        let dy = targetY - npc.pos.y;
        ...
        if (Math.abs(dx) >= Math.abs(dy)) {
            if (!this.isBlocked(npc.pos.x + stepX, npc.pos.y)) newX += stepX;
            else if (dy !== 0 && !this.isBlocked(npc.pos.x, npc.pos.y + stepY)) newY += stepY;
        } else {
            if (!this.isBlocked(npc.pos.x, npc.pos.y + stepY)) newY += stepY;
            else if (dx !== 0 && !this.isBlocked(npc.pos.x + stepX, npc.pos.y)) newX += stepX;
        }
    ...
    private handleMove(dx: number, dy: number): GameEvent[] {
        ...
        if (this.isBlocked(newX, newY)) {
            return [{ type: 'LOG', text: "Путь прегражден.", logType: LogType.ERROR }];
        }
        ...
        const events: GameEvent[] = [];
        events.push({ type: 'ENTITY_MOVED', text: "", logType: LogType.INFO, data: { actorId: p.id, x: newX, y: newY } });

        // Check floor items
        const item = this.getEntityAt(newX, newY);
        if (item) {
            if (item.type === EntityType.ITEM) {
                events.push({ type: 'LOG', text: `Вы видите: ${item.name}.`, logType: LogType.INFO });
            } else if (item.type === EntityType.EXIT) {
                const action = item.symbol === SYMBOLS.EXIT ? 'спуск' : 'наверх';
                events.push({ type: 'LOG', text: `${item.name}. Введите '${action}'.`, logType: LogType.INFO });
            }
        }
    ...
    private handleAttack(targetId: string): GameEvent[] {
        ...
        if (!target) return [{ type: 'LOG', text: "Цель не найдена.", logType: LogType.ERROR }];
        if (p.stats.stamina < STAMINA_COST.ATTACK) return [{ type: 'LOG', text: "Нет сил на удар!", logType: LogType.ERROR }];

        p.stats.stamina -= STAMINA_COST.ATTACK;

        const damage = Math.max(1, p.stats.strength + Math.floor(Math.random() * 3));
        ...
        if (this.gameState === GameState.EXPLORATION && target.isHostile) {
            ...
            events.push({ type: 'GAME_STATE_CHANGE', text: "НАЧАЛСЯ БОЙ!", logType: LogType.COMBAT });
        }
    ...
    private handlePickup(): GameEvent[] {
        const p = this.player!;
        const item = this.entities.find(e => e.type === EntityType.ITEM && e.pos.x === p.pos.x && e.pos.y === p.pos.y);

        if (!item || !item.itemData) return [{ type: 'LOG', text: "Здесь нечего брать.", logType: LogType.INFO }];
        ...
        if (data.type === ItemType.GOLD) {
            ...
            return [{ type: 'ITEM_PICKUP', text: `Вы нашли ${data.value} золота.`, logType: LogType.SUCCESS }];
        } else {
            p.inventory.push(data);
            return [{ type: 'ITEM_PICKUP', text: `Вы подобрали: ${data.name}.`, logType: LogType.SUCCESS }];
        }
    }

    private handleUse(itemName: string): GameEvent[] {
        const p = this.player!;
        const idx = p.inventory.findIndex(i => i.name.toLowerCase().includes(itemName.toLowerCase()));

        if (idx === -1) return [{ type: 'LOG', text: "Нет такого предмета.", logType: LogType.ERROR }];

        const item = p.inventory[idx];
        if (item.type === ItemType.POTION) {
            p.stats.hp = Math.min(p.stats.maxHp, p.stats.hp + item.value);
            p.inventory.splice(idx, 1);
            return [{ type: 'HEAL', text: `Вы выпили ${item.name}. +${item.value} HP.`, logType: LogType.SUCCESS }];
        }

        return [{ type: 'LOG', text: "Нельзя использовать это.", logType: LogType.ERROR }];
    }

    private handleStairs(dir: 'up' | 'down'): GameEvent[] {
        const p = this.player!;
        const stairs = this.entities.find(e => e.type === EntityType.EXIT && e.pos.x === p.pos.x && e.pos.y === p.pos.y);

        if (!stairs) return [{ type: 'LOG', text: "Здесь нет лестницы.", logType: LogType.ERROR }];

        if (dir === 'down') {
            if (stairs.symbol === SYMBOLS.EXIT) { // Down logic
                return this.init(this.world!.level + 1, p);
            }
            return [{ type: 'LOG', text: "Это лестница вверх.", logType: LogType.ERROR }];
        } else {
            if (stairs.symbol === SYMBOLS.EXIT_UP) { // Up logic
                return this.init(Math.max(0, this.world!.level - 1), p);
            }
            return [{ type: 'LOG', text: "Это лестница вниз.", logType: LogType.ERROR }];
        }
    }
    ...
    private handleBuy(): GameEvent[] {
        ...
        if (!merchant || merchant.npcType !== 'MERCHANT') return [{ type: 'LOG', text: "Здесь нет торговца.", logType: LogType.ERROR }];

        if (p.stats.gold < PRICES.POTION) return [{ type: 'LOG', text: "Не хватает золота.", logType: LogType.ERROR }];

        p.stats.gold -= PRICES.POTION;
        p.inventory.push({ 
            id: Math.random().toString(), name: 'Зелье Лечения', type: ItemType.POTION, value: 20, description: 'HP +20' 
        });

        return [{ type: 'TRANSACTION', text: "Вы купили зелье.", logType: LogType.SUCCESS }];
    }

    private handleHeal(): GameEvent[] {
        const p = this.player!;
        const healer = this.getNearbyNPC();
        if (!healer || healer.npcType !== 'HEALER') return [{ type: 'LOG', text: "Здесь нет лекаря.", logType: LogType.ERROR }];

        if (p.stats.gold < PRICES.HEAL) return [{ type: 'LOG', text: "Нужно больше золота.", logType: LogType.ERROR }];

        p.stats.gold -= PRICES.HEAL;
        p.stats.hp = p.stats.maxHp;

        return [{ type: 'HEAL', text: "Ваши раны исцелены.", logType: LogType.SUCCESS }];
    }

    private handleLook(): GameEvent {
        if (this.world?.level === 0) {
            return { type: 'LOG', text: "Вы в городе. Безопасно.", logType: LogType.INFO };
        }
        
        const visible = this.getVisibleEntities();
        if (visible.length === 0) return { type: 'LOG', text: "Осмотр: Темно и пусто.", logType: LogType.INFO };
    ...
    private checkCombatState(events: GameEvent[]) {
        if (this.world?.level === 0) return; // No combat in town

        const visibleHostiles = this.getVisibleEntities().filter(e => e.isHostile && !e.isDead);
        
        if (visibleHostiles.length === 0 && this.gameState === GameState.COMBAT) {
            this.gameState = GameState.EXPLORATION;
            events.push({ type: 'GAME_STATE_CHANGE', text: "Бой окончен.", logType: LogType.INFO });
        }
    }
    ...
    private updateVisibility() {
        ...
        // Reset visibility
        this.world.map.forEach(row => row.forEach(tile => tile.isVisible = false));
    ...
    public getContextDescription(): string {
        if (this.world?.level === 0) return "Локация: Тихая Гавань. Мирный город.";
        const visible = this.getVisibleEntities();
        if (visible.length === 0) return "Темный коридор подземелья.";
        
        const details = visible.map(e => {
            let desc = `[${e.label}] ${e.name}`;
            if (e.npcType) desc += ` (Роль: ${e.npcType})`;
            if (e.isHostile) desc += ` (Враг, HP: ${e.stats.hp})`;
            return desc;
        }).join(', ');
    ...
    private isBlocked(x: number, y: number): boolean {
        ...
        if (tile.isWall || tile.env === 'water') return true;
        // Check if blocked by another entity (unless it's an item/exit)
        const blocker = this.entities.find(e => e.pos.x === x && e.pos.y === y && !e.isDead && e.type !== EntityType.ITEM && e.type !== EntityType.EXIT);
        return !!blocker;
    }

    private getHostileAt(x: number, y: number): Entity | undefined {
        return this.entities.find(e => e.pos.x === x && e.pos.y === y && !e.isDead && e.isHostile);
    }

    private getEntityAt(x: number, y: number): Entity | undefined {
        // Priority: Item/Exit
        return this.entities.find(e => e.pos.x === x && e.pos.y === y && (e.type === EntityType.ITEM || e.type === EntityType.EXIT));
    }

    private getNearbyNPC(): Entity | undefined {
        const p = this.player!;
        return this.entities.find(e => e.type === EntityType.NPC && Math.abs(e.pos.x - p.pos.x) <= 1.5 && Math.abs(e.pos.y - p.pos.y) <= 1.5);
    }
    ...
    private hasLineOfSight(x0: number, y0: number, x1: number, y1: number): boolean {
        // Bresenham's Line
        if (this.world?.level === 0) return true;
        let dx = Math.abs(x1 - x0);
        let dy = Math.abs(y1 - y0);
        let sx = (x0 < x1) ? 1 : -1;
        let sy = (y0 < y1) ? 1 : -1;
        ...
        while (true) {
            ...
            if (this.world!.map[cy][cx].isWall && (cx !== x0 || cy !== y0)) return false;
            let e2 = 2 * err;
            if (e2 > -dy) { err -= dy; cx += sx; }
            if (e2 < dx) { err += dx; cy += sy; }
        }
